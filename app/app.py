...
# Mantém somente os ícones nativos definidos em st.Page.
# Os estilos anteriores criavam um segundo ícone com ::before.
st.markdown(
    """
    <style>
...
with st.sidebar:
    ...
    st.info(
        "Explore os dados, realize uma predição e consulte o desempenho "
        "do modelo pelas páginas do menu."
    )
...
DIRETORIO_PAGINAS = Path(__file__).resolve().parent / "pages"

# Compatibilidade com o nome original e com a codificação observada no ZIP.
CAMINHO_PREDICAO = next(
    (
        caminho
        for caminho in (
            DIRETORIO_PAGINAS / "3_Predição.py",
            DIRETORIO_PAGINAS / "3_Predi#U00e7#U00e3o.py",
        )
        if caminho.exists()
    ),
    DIRETORIO_PAGINAS / "3_Predição.py",
...
paginas = [
    ...
    st.Page(
        DIRETORIO_PAGINAS / "1_Projeto.py",
        title="Projeto",
        icon=":material/description:",
    ),
    st.Page(
        DIRETORIO_PAGINAS / "2_Dashboard.py",
        title="Dashboard",
    ...
    st.Page(
        DIRETORIO_PAGINAS / "4_Modelo.py",
        title="Modelo",
        icon=":material/model_training:",
    ),
    st.Page(
        DIRETORIO_PAGINAS / "5_Equipe.py",
        title="Equipe",
